...
fn main() {
    ...
    let bindings = bindgen::Builder::default()
        ...
        // Newer glibc versions (2.44+ as seen on Arch) trip up bindgen's layout
        // computation for glibc's internal stdio structs, producing a bindings.rs
        // that fails to compile (a `size_of::<_IO_FILE>()` layout-test assertion
        // mismatch). Only opaque `FILE*` pointers ever cross the FFI boundary, so
        // treating it as opaque sidesteps the bug without losing functionality.
