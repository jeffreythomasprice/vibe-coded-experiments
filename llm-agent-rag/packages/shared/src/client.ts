...
export class RagClient {
  ...
  async findDocuments(tags: Record<string, string>): Promise<DocumentSummary[]> {
    return this.jsonPost<DocumentSummary[]>("/api/documents/find", { tags });
  }

  async uploadFile(file: File, tags?: Record<string, string>): Promise<IngestResult> {
  ...
  async query(query: string, topK?: number, tags?: Record<string, string>): Promise<ChunkResult[]> {
    return this.jsonPost<ChunkResult[]>("/api/query", { query, top_k: topK, tags });
  }

  async ask(query: string, topK?: number, tags?: Record<string, string>): Promise<AskResponse> {
