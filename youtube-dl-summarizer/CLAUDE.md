...
- **claude** (Claude Code CLI) — used for summarization via `claude -p`
...
3. **Summarize** (`steps/summarize.ts`) — sends transcript to Claude CLI (`claude -p`), produces markdown with `[HH:MM:SS]` timestamps
