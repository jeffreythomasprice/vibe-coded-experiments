...
/// Writes `png_bytes` (as returned by `sd-server`, which embeds a generation-
/// params tag in them when asked) to `dest`. When `dest`'s extension is PNG
/// this is a direct byte-for-byte write, preserving that tag; any other
/// extension is decoded and re-encoded via `image`, which drops it.
pub fn write_image(dest: &Path, png_bytes: &[u8]) -> Result<(), AppError> {
    ...
    image::load_from_memory(png_bytes)?.save(dest)?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn write_image_writes_png_bytes_directly() {
        let dir = tempfile::tempdir().unwrap();
        let dest = dir.path().join("out.png");

        write_image(&dest, b"fake png bytes").unwrap();

        assert_eq!(std::fs::read(&dest).unwrap(), b"fake png bytes");
    }

    #[test]
    fn write_image_re_encodes_for_a_non_png_extension() {
        let dir = tempfile::tempdir().unwrap();
        ...
        write_image(&dest, &png_bytes).unwrap();
    ...
    #[test]
    fn write_image_treats_a_missing_extension_as_png() {
        let dir = tempfile::tempdir().unwrap();
