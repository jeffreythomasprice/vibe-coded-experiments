...
use shared::battle::{ActionKind, BattleMode, SequenceKind};
    ...
    pub fn entry(self) -> Entry {
        match self {
            ...
            Topic::RoomEveryoneWrites => Entry {
                term: "Everyone who joins can edit",
                what: "Decides whether someone joining can change the battle straight away, or only watch until a writer says otherwise.",
                interacts: "It only sets the starting point for each new joiner \u{2014} it's never retroactive to anyone already in the room. Once someone's in, anyone with write access can grant or revoke anyone else's, except their own and the host's. This can be changed at any time by anyone who currently has write access, and takes effect for the next person who joins.",
                source: Source::AppConvention,
            },
            Topic::RoomRoleHost => Entry {
                term: "Host",
                what: "This is the host \u{2014} whoever created this room.",
                interacts: "The host always has write access and can never be demoted or kicked, by anyone, including themselves. If the host's connection drops, the room keeps running with whoever else is in it; nobody else ever becomes host in their place.",
                source: Source::AppConvention,
            },
            Topic::RoomRoleWriter => Entry {
                term: "Can edit",
                what: "This player can change the battle.",
                interacts: "They can add and remove combatants, declare actions, advance the tick, and undo \u{2014} and they can grant or revoke anyone else's write access, except their own and the host's. The server checks every change they make is actually legal before applying it, the same as it does for anyone.",
                source: Source::AppConvention,
            },
            Topic::RoomRoleReader => Entry {
                term: "Read-only",
                what: "This player can watch every change as it happens, but can't make any.",
                interacts: "They can still rename themselves, leave, and ask the server to resend the current state \u{2014} everything else that would change the battle is refused by the server, so the controls for it are greyed out here too. Anyone with write access can grant them some at any time, and it takes effect immediately.",
                source: Source::AppConvention,
            },
            Topic::RoomCanWrite => Entry {
                term: "Can edit",
                what: "Whether this player may change the battle.",
                interacts: "Anyone who already has it can grant or revoke it for anyone else, but never for themselves and never for the host. It takes effect the moment the server's next update reaches this browser: the affected player's controls grey out or light up without them doing anything.",
                source: Source::AppConvention,
            },
            Topic::RoomRename => Entry {
                term: "Your name",
                what: "The name you're listed under in this room's member list.",
                interacts: "Change it whenever you like \u{2014} it's sent to the room as soon as you leave the field or press Enter. It's only a label: it has nothing to do with any combatant in the battle, and changing it doesn't affect what you're allowed to do.",
                source: Source::AppConvention,
            },
            ...
            Topic::RoomAdminList => Entry {
                term: "All rooms",
                what: "Every room currently live on the server, searchable by name.",
                interacts: "Only shown when your own access code is an admin. A blank search shows everything. Rooms aren't listed forever -- one with nobody actively using it drops off here on its own after 30 idle minutes, the same TTL that governs the room itself.",
                source: Source::AppConvention,
            },
            Topic::RoomAdminClose => Entry {
                term: "Close room",
                what: "Deletes this room's battle from the server and disconnects everyone currently in it.",
                interacts: "Asks for confirmation first, since it can't be undone -- the room would have to be hosted again from scratch. Everyone who was in it drops to Solo keeping their own local copy of the battle, and sees a message that an administrator closed their room.",
                source: Source::AppConvention,
            },
            Topic::Config => Entry {
                term: "Settings",
                what: "Checks or sets the access code this browser uses to talk to the server, and — for an admin code — manages every code the server knows about.",
                interacts: "Every request this app makes, including every multiplayer message, is authenticated with this code \u{2014} but \"admin\" here means server administration (who may manage access codes), unrelated to write access inside any one room.",
                source: Source::AppConvention,
            },
            Topic::ConfigCode => Entry {
                term: "Access code",
                what: "The credential this browser sends to the server with every request, as a bearer token.",
                interacts: "The server checks it before anything else \u{2014} an unrecognized code is rejected and nothing is saved. Once accepted, it's kept in this browser only and sent with every request until you clear it.",
                source: Source::AppConvention,
            },
            Topic::ConfigSelf => Entry {
                term: "You",
                what: "The row for the access code this browser is currently signed in as.",
                interacts: "The server refuses to let an admin demote or delete their own code \u{2014} it's the only way to guarantee there's always at least one admin left \u{2014} so this row's Admin checkbox and delete button are unavailable here even though they work on every other row.",
                source: Source::AppConvention,
            },
            Topic::ConfigTable => Entry {
                term: "All access codes",
                what: "Every access code the server currently accepts, newest first.",
                interacts: "Only shown when your own code is an admin \u{2014} anyone with an admin code can see, create, promote, demote, or delete any code here, including ones other people are actively using.",
                source: Source::AppConvention,
            },
            Topic::ConfigAdminFlag => Entry {
                term: "Admin",
                what: "Whether this access code may manage access codes — see every code, create new ones, and promote, demote, or delete existing ones.",
                interacts: "Takes effect on that code's very next request. This is server administration, unrelated to write access inside a multiplayer room, which is granted per connection, not per access code.",
                source: Source::AppConvention,
            },
            Topic::ConfigCreate => Entry {
                term: "New access code",
                what: "Adds a code the server will accept.",
                interacts: "Type a value to choose it yourself, or leave it blank to have the server generate a random one. Either way it must be unique — a value already in use is rejected.",
                source: Source::AppConvention,
            },
            Topic::ConfigDelete => Entry {
                term: "Delete",
                what: "Immediately revokes an access code. Anyone using it loses access on their very next request.",
                interacts: "Asks for confirmation first, since it can't be undone — a deleted code would have to be created again, as a new one. An admin can't delete their own code; see the \"You\" row.",
                source: Source::AppConvention,
            },
...
mod tests {
    ...
    const ALL: &[Topic] = &[
        ...
        Topic::RoomRoleReader,
        Topic::RoomCanWrite,
        Topic::RoomRename,
        Topic::RoomKick,
        Topic::RoomInviteLink,
        Topic::RoomAdminList,
        Topic::RoomAdminClose,
        Topic::Config,
        Topic::ConfigCode,
        Topic::ConfigSelf,
        Topic::ConfigTable,
        Topic::ConfigAdminFlag,
        Topic::ConfigCreate,
        Topic::ConfigDelete,
        Topic::Roster,
        Topic::CombatantName,
